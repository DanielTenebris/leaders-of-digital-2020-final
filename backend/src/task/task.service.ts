...
export class TaskService {
  ...
  async create(task: CreateTaskDto): Promise<TaskEntity> {
    ...
    newTask.contracts = await Promise.all(
      task.contractIds.map(
        async contract => await this.contractService.findOneById(contract),
      ),
    );
  ...
  async updateOne(
    taskId: number,
    updateTask: DeepPartial<UpdateTaskDto>,
    exec = true,
  ): Promise<TaskEntity> {
  ...
  async execContracts(taskId: number): Promise<any> {
    ...
    const taskWithRelatedContracts = await this.taskRepository.findOne({
      where: { id: taskId },
      relations: ['contracts'],
    });
    ...
    const functionsByEmited = {
      ...
      новая_задача: () => {
        const newTask = {
          title: 'Новая Задача',
          desc: 'Задача созданная через контракт',
          deadline: new Date('2020-12-30 10:59:14.664926'),
          workerId: [521, 787],
          createdById: 1,
          contractIds: [],
        };
