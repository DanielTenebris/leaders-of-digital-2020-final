...
export class ContractService {
    ...
    async updateOne(contractId: number, updateContract: DeepPartial<UpdateContractDto>): Promise<ContractEntity> {
    ...
    async execContracts(taskId: number): Promise<any> {
        ...
        const emitEvents = []
        const contracts = await this.contractRepository.find({ relations: ['tasks'] } );
